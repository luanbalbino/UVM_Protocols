class spi_scb#(parameter int WORD_LEN = 12) extends uvm_scoreboard;
    ...
    uvm_tlm_analysis_fifo #(spi_seq_item#(WORD_LEN)) rtl_out;
    uvm_tlm_analysis_fifo #(spi_seq_item#(WORD_LEN)) tr_in;

    spi_seq_item#(WORD_LEN) spi_seq_out;
    spi_seq_item#(WORD_LEN) spi_seq_in;
    ...
    function void compare(spi_seq_item expected, spi_seq_item actual);
        ...
        if (expected.mosi_data !== actual.mosi_data)
            `uvm_error("SCOREBOARD", $sformatf("Mismatch! expected: %h, actual: %h", expected.mosi_data, actual.mosi_data))
        else
